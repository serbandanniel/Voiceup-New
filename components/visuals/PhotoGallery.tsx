    ...
    return (
        <div className="max-w-screen-xl mx-auto px-4 md:px-6 w-full">
            <div className="text-center mb-10">
                ...
                <p className="mt-4 text-gray-600">Amintiri de neuitat de la edițiile anterioare.</p>
            </div>

            {/* BENTO GRID LAYOUT */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 auto-rows-[180px] w-full">
                
                {images.map((img, index) => (
                    <div 
                        key={img.id} 
                        className={`relative group rounded-2xl md:rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100 cursor-pointer w-full h-full ${getGridClass(index)}`}
                ...
                {/* The "See More" Card */}
                <div 
                    className="relative rounded-2xl md:rounded-3xl overflow-hidden shadow-lg cursor-pointer group col-span-2 row-span-1 md:col-span-1 md:row-span-1 w-full h-full"
                    onClick={() => navigate('/galerie')}
                >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-purple/90 to-brand-pink/90 group-hover:from-brand-purple/80 group-hover:to-brand-pink/80 transition-all duration-300 flex flex-col items-center justify-center text-white p-2 text-center">
                        <span className="text-3xl md:text-4xl font-black drop-shadow-md group-hover:scale-110 transition-transform duration-300">+</span>
                        <span className="text-xs md:text-xs font-bold uppercase tracking-widest mt-1 opacity-90 border-b border-white/30 pb-0.5">Vezi Toată Galeria</span>
                    </div>
                </div>
            </div>
        </div>
